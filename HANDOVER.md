# PDF Studio Project Handover (v8.0.9)
...
* **PWAキャッシュの注意**:
  * コード修正時は、ブラウザの強力なPWAキャッシュを更新するため、`index.html`、`sw.js`、`js/app.js` のバージョン番号（例: `v8.0.8` -> `v8.0.9`、`v408` -> `v409`）を必ずインクリメントしてください。

---

## 5. 現在の稼働ステータス (v8.0.9)
* **最新バージョン**: v8.0.9 (安定稼働中)
* **対応完了環境**: PC (Chrome/Edge), iPad (Safari/PWA), iPhone (Safari/PWA)
* **Google Drive連携**:
  * OAuthトークン自動更新機能付きで安定動作。
  * **マイドライブ最上位（root）起点フォルダ階層ナビゲーション** に対応。直下フォルダから順次フォルダを潜ってPDFを選択可能。
