# PDF Studio Project Handover (v9.0.2)
...
* **PWAキャッシュの注意**:
  * コード修正時は、ブラウザの強力なPWAキャッシュを更新するため、`index.html`、`sw.js`、`js/app.js` のバージョン番号（例: `v9.0.2` -> `v9.0.3`、`v502` -> `v503`）を必ずインクリメントしてください。

---

## 5. 現在の稼働ステータス (v9.0.2)
* **最新バージョン**: v9.0.2 (完全安定稼働中)
* **対応完了環境**: PC (Chrome/Edge/PWA), iPad (Safari/PWA), iPhone (Safari/PWA)
* **Google Drive専用内蔵ブラウザ (v9.0.2)**:
